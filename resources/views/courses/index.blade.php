        ...
        @if ($liveProgramCourses->isNotEmpty())
            ...
            <div class="mb-18 grid grid-cols-1 gap-6 lg:grid-cols-3">
                @foreach ($liveProgramCourses as $course)
                    <a href="{{ route('courses.show', $course) }}" wire:navigate class="flex flex-col gap-[14px] bg-crema-suave p-8">
                        <h3 class="text-xl font-bold">{{ $course->title }}</h3>
                        <p class="grow text-[15px] leading-[1.6] opacity-[0.85]">{{ Str::limit(strip_tags($course->description), 140) }}</p>
                        <div class="text-xl font-bold text-terracota">${{ number_format($course->price_cents / 100, 0) }}</div>
                    </a>
                @endforeach
            </div>
        @endif
        ...
        @if ($hybridCourses->isNotEmpty())
            ...
            <div class="mb-18 grid grid-cols-1 gap-6">
                @foreach ($hybridCourses as $course)
                    <a href="{{ route('courses.show', $course) }}" wire:navigate class="flex flex-col items-start gap-6 border-t-[3px] border-terracota bg-crema-suave p-9 sm:flex-row sm:items-end sm:justify-between">
                        ...
                        <div class="text-[22px] font-bold text-terracota">${{ number_format($course->price_cents / 100, 0) }}</div>
                    </a>
                @endforeach
            </div>
        @endif
        ...
        @if ($recordedCourses->isNotEmpty())
            ...
            <div class="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-5">
                @foreach ($recordedCourses as $course)
                    <a href="{{ route('courses.show', $course) }}" wire:navigate class="flex flex-col gap-3 border-[1.5px] border-espresso p-[26px]">
                        ...
                        <div class="text-lg font-bold text-terracota">${{ number_format($course->price_cents / 100, 0) }}</div>
                    </a>
                @endforeach
            </div>
        @endif
